...
export default function Ecosystem() {
  ...
  async function runSync() {
    ...
    try {
      ...
      toast.success(`Synced • ${counts?.segments || 0} segments · ${counts?.companies || 0} companies · ${counts?.competitors || 0} competitors · ${counts?.partners || 0} partners · ${counts?.whitespace || 0} whitespace · ${counts?.roles || 0} roles · ${counts?.people || 0} people · ${counts?.themes || 0} themes · ${counts?.insights || 0} insights`);
      setRefreshKey((k) => k + 1);
    } catch (e: any) {
